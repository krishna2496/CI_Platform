...
if (count($tenants) > 0) {
    foreach ($tenants as $tenant) {
        ...
        $availabilities = $pdo->query('select availability_id, translations from availability')->fetchAll(PDO::FETCH_ASSOC);
        ...
        if (!empty($availabilities)) {
            foreach ($availabilities as $availability) {
                ...
                $trans = [];
                foreach ($availabilityTranslations as $translation) {
